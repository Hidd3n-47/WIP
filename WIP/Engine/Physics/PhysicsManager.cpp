...
void PhysicsManager::Update(float dt)
{
	...
	{
		if (impulses[i].m_acceleration > 0.1f)
		{
			vec2* position = impulses[i].GetEntity()->GetComponent<Transform>()->GetPositionPointer();

			impulses[i].m_velocity -= glm::normalize(impulses[i].m_velocity) * impulses[i].m_acceleration;

			*position += impulses[i].m_velocity * dt;
			impulses[i].m_acceleration *= impulses[i].m_dampeningFactor;
		}
	}
}
